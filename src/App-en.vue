  ...
  <div id="app">
    ...
    <div v-if="helped" class="popup active" v-on:click="() => { this.helped = false }">
      <div>
        ...
        <p>Guess the hidden Pokémon and catch it. You have 5 attempts.</p>
        <p><strong>A new Pokémon every day!</strong></p>
        <p class="small">Version {{ version }}</p>
        <p class="small">Pokémon and Pokémon character names are trademarks of Nintendo.</p>
      </div>
    </div>
    ...
    <div v-if="intents && !pokemon.active">
      <div>
        <input v-model="input" placeholder="What Pokémon is it?" class="textInput" :class="{'error-shake': errorShake}" readonly />
    ...
    <div v-if="win && !showPokedex" class="msg">
      <div>
        ...
        <p>You have caught your daily Pokémon.</p>
    ...
    <div v-if="!win && !intents && !showPokedex" class="msg">
      <div>
        <h2 class="title">It has escaped!</h2>
        <p>Come back tomorrow to find another.</p>
        <button @click="setPokedex" class="btn">Pokédex</button>
      </div>
    </div>
