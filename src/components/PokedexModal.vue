<template>
  <div>
    <button @click.prevent="hidePokedex = !hidePokedex" :class="{ tab: true, active: !hidePokedex }">
      Pokédex
    </button>
    <div :class="{ pokedex: true, active: !hidePokedex }">
      ...
      <div class="list">
        <div v-for="(pokemon, index) in pokedex" v-bind:key="index" :class="{ pokemon: true, active: pokemon.active }"
          ...
          <img loading="lazy" :srcset="'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/' +
            (index + 1) +
            '.png'
            " alt="" />
          <h2 class="name">
