  ...
  <section>
    <div v-if="win" class="msg">
      <div>
        ...
        <div class="share">
          <a :href="`https://twitter.com/intent/tweet?url=https%3A%2F%2Fpokemodle.salteadorneo.dev%2F&text=${encodeURIComponent(
            shareText
          )}`" target="_blank" @click="setEvent('twitter')">
            <TwitterIcon />
          </a>
          <a :href="`https://api.whatsapp.com/send?text=${encodeURIComponent(
            shareText + ' https://pokemodle.salteadorneo.dev/'
          )}`" target="_blank" @click="setEvent('whatsapp')">
            <WhatsappIcon />
          </a>
          <a :href="`https://telegram.me/share/url?url=https%3A%2F%2Fpokemodle.salteadorneo.dev%2F&text=${encodeURIComponent(
            shareText
          )}`" target="_blank" @click="setEvent('telegram')">
            <TelegramIcon />
          </a>
          <button @click="clipboard" class="btn rounded">
            <CopyIcon />
          </button>
        </div>
